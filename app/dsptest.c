...
void matrix_init(arm_matrix_instance_f32* mat, arm_matrix_instance_f32* mat2)
{
	arm_mat_init_f32(mat, 4, 4, (float32_t *)A_f32); 
...
{ 
	...
	arm_mat_init_f32(mat2, 4, 4, (float32_t *)tmpf32);
}
	
void matrix_cmp(void)
{
	uint32_t timestart, timediff;
	
	//dsp library matrix
	arm_matrix_instance_f32 armMA;
	arm_matrix_instance_f32 armMB;
	arm_matrix_instance_f32 armMtmp;
	uint32_t srcRows, srcColumns;
	arm_mat_init_f32(&armMA, srcRows, srcColumns, (float32_t *)A_f32); 
	arm_mat_init_f32(&armMB, srcRows, srcColumns, (float32_t *)A_f32); 
	
	//our C matrix
	MATRIX4X4 ma, mb, mtmp; 
	Mat_Init_4X4(&ma, 	1.0, 		32.0, 		4.0, 		128.0,
						1.0, 		32.0, 		64.0, 		2048.0,
						1.0, 		16.0, 		4.0, 		64.0,
						1.0, 		16.0, 		64.0, 		1024.0);
	Mat_Init_4X4(&mb, 	1.0, 		32.0, 		4.0, 		128.0,
						1.0, 		32.0, 		64.0, 		2048.0,
						1.0, 		16.0, 		4.0, 		64.0,
						1.0, 		16.0, 		64.0, 		1024.0);
	
	//loop 10000 cal matrix mult
	...
	{
		arm_mat_mult_f32(&armMA, &armMB, &armMtmp);
	}
	timediff = xTaskGetTickCount() - timestart;

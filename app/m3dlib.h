...
//	float32_t avg_radius;  // average radius of object used for collision detection
//	float32_t max_radius;  // maximum radius of object
...
//	VECTOR4D target; // look at target
	...
	float32_t view_dist;  // focal length 

	float32_t fov;          // field of view for both horizontal and vertical axes

	// 3d clipping planes
	// if view volume is NOT 90 degree then general 3d clipping
	// must be employed
	float32_t near_clip_z;     // near z=constant clipping plane
	float32_t far_clip_z;      // far z=constant clipping plane
	...
	float32_t viewplane_width;     // width and height of view plane to project onto
	float32_t viewplane_height;    // usually 2x2 for normalized projection or 
							   // the exact same size as the viewport or screen window

	// remember screen and viewport are synonomous 
	float32_t viewport_width;     // size of screen/viewport
	float32_t viewport_height;
	float32_t viewport_center_x;  // center of view port (final image destination)
	float32_t viewport_center_y;
...
void Build_XYZ_Rotation_MATRIX4X4(float32_t theta_x, float32_t theta_y, float32_t theta_z,
                                  MATRIX4X4_PTR mrot);
...
void Init_CAM4DV1(CAM4DV1_PTR cam, int32_t attr, POINT4D_PTR cam_pos, 
                  ...
                  float32_t near_clip_z, float32_t far_clip_z, float32_t fov, 
                  float32_t viewport_width,  float32_t viewport_height);

void Adjust_Viewport_CAM4DV1(CAM4DV1_PTR cam,       // the camera object
                  float32_t viewport_width,  // size of final screen viewport
                  float32_t viewport_height);

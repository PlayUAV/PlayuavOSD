...
///////////////////////////////////////////////////////////////
void Build_XYZ_Rotation_MATRIX4X4(float32_t theta_x, // euler angles
                                  float32_t theta_y, 
                                  float32_t theta_z,
                                  MATRIX4X4_PTR mrot) // output 
{
	...
	float32_t sin_theta=0, cos_theta=0;   // used to initialize matrices
...
{
	float32_t alpha = (0.5*cam->viewport_width-0.5);
	float32_t beta  = (0.5*cam->viewport_height-0.5);
	...
	for (int32_t poly=0; poly < obj->num_polys; poly++)
	{
		...
		float32_t dp = VECTOR4D_Dot(&n, &view);
...
void Build_CAM4DV1_Matrix_Euler(CAM4DV1_PTR cam)
{
	...
	float32_t theta_x = cam->dir.x;
	float32_t theta_y = cam->dir.y;
	float32_t theta_z = cam->dir.z;

	// compute the sine and cosine of the angle x
	float32_t cos_theta = Fast_Cos(theta_x);  // no change since cos(-x) = cos(x)
	float32_t sin_theta = -Fast_Sin(theta_x); // sin(-x) = -sin(x)
...
void Init_CAM4DV1(CAM4DV1_PTR cam,       // the camera object
                  ...
                  float32_t near_clip_z,     // near and far clipping planes
                  float32_t far_clip_z,
                  float32_t fov,             // field of view in degrees
...
//		VECTOR4D_ZERO(&cam->target);
	...
	cam->aspect_ratio = (float32_t)viewport_width/(float32_t)viewport_height;
	...
	float32_t tan_fov_div2 = tan(DEG_TO_RAD(fov/2));
                  ...
                  float32_t viewport_height)
{
	...
	// now we know fov and we know the viewplane dimensions plug into formula and
	// solve for view distance parameters
	float32_t tan_fov_div2 = tan(DEG_TO_RAD(cam->fov/2));
